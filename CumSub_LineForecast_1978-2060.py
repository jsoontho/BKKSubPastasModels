...
# Forecasts of cumulative subsidence for a well nest based on four
# pumping scenarios

# Recommends running code for one well at a time (16 min run time 
# for one well nest)

# Output: Forecast of cumulative subsidence in a line plot from 1978-2060
    ...
    for num_well, wellnest in enumerate(wellnestlist):
        ...
        if benchflag == 1:
            ...
            try:
                ...
                subdata = subdata[(subdata.Year>=int(tmin)) & (subdata.Year<=int(tmax))]
                subdata = subdata[(subdata.Year>=int(date.year[0])) & \
                                  (subdata.Year<=int(date.year[-1]))]
        ...
        # Plotting benchmarks
        if np.logical_and(wellnest != "TEST", benchflag == 1):
            
            if not bench.empty:
                # Measurements
                plt.plot(bench.index, np.cumsum(bench.iloc[:, 0]), "-o", color="orange",
                          label="Leveling")
...
for num_well, wellnest in enumerate(wellnestlist):
    ...
    lastrate = (all_ann_subs[0][num_well][1].CumTotSum[-1] - \
            all_ann_subs[0][num_well][1].CumTotSum[-2])*100
    ann_2060_500.append(lastrate)
    ax.annotate('{:.3f}'.format(lastrate), 
                xy=(180, 13.2),
    ...
    lastrate = (all_ann_subs[1][num_well][1].CumTotSum[-1] - \
            all_ann_subs[1][num_well][1].CumTotSum[-2])*100
    ...
    lastrate = (all_ann_subs[3][num_well][1].CumTotSum[-1] - \
            all_ann_subs[3][num_well][1].CumTotSum[-2])*100
    ...
    lastrate = (all_ann_subs[2][num_well][1].CumTotSum[-1] - \
            all_ann_subs[2][num_well][1].CumTotSum[-2])*100
    ...
    lastrate = (all_ann_subs[4][num_well][1].CumTotSum[-1] - \
            all_ann_subs[4][num_well][1].CumTotSum[-2])*100
    ann_2060_0.append(lastrate)
    ax.annotate('{:.3f}'.format(lastrate), 
                xy=(180, 33),
                xycoords='axes points')
    plt.plot(all_ann_subs[4][num_well][1].index[:44], 
             all_ann_subs[4][num_well][1].CumTotSum.iloc[:44]*100, 
    ...
    plt.title(wellnest + \
              "\nSimulated Cumulative Subsidence")
    plt.show()
    
    # Saving figure
    fig_name = wellnest + "_Clay_AllHeadDeform_ALLPUMP.png"
    full_figpath = os.path.join(path, fig_name)
