...
fig_path = os.path.join(abs_path, 'figures')
...
for Wellnest_name in files:
    ...
    for wells in data.columns[-(len(data.columns)-2):]:
        ...
        max_obs = max(model.observations())
        min_obs = min(model.observations())
...
for aq in aqs: 
    ...
    for x,y in zip(gwwell_locs.Long, gwwell_locs.Lat):
        ...
        if (x, y) not in unique:
            ...
            label = gwwell_locs.loc[gwwell_locs.Long == x]['WellNest_Name'].tolist()
...
data_lim = [min(np.array(data_temp)[~js_mfs.is_outlier(data_temp, 3.5)]),
            max(np.array(data_temp)[~js_mfs.is_outlier(data_temp, 3.5)])]
...
js_mfs.draw_basemap(map, xs, ys, d_dict, fig=fig, ax=ax, 
                    datalim=data_lim, mode='RMSE_full', save=0, 
                    aq=aq, perc=0,
                    time_min=time_min, time_max=time_max, figpath=fig_path)
