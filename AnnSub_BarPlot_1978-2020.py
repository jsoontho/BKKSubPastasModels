...
class SolveFDM:
    ...
    def __init__(self, Nz, n, dz, Kv, Sskv, Sske, dt, precon, CC, toplay):
    ...
    def elastic_inelastic(self, h_matr):
        ...
        for i in range(1, self.Nz+1):
            ...
            # All other time steps
                ...
                # slope is negative
                if np.logical_and(
                        h_matr[i, self.n-1] <= self.precon[i], dh < 0):
    ...
    def build_coeff_matrix(self, h_matr):
        """Build A matrix.
        ...
        Ss, precon = self.elastic_inelastic(h_matr)
        ...
        for i in range(1, self.Nz-1):
            Adiag_val[i] = (-2 * self.Kv / self.dz) - \
                (self.dz / self.dt * Ss[i+1])
        ...
        if self.toplay == False:
            Adiag_val[0] = (-3 * self.Kv / self.dz) - \
                (self.dz / self.dt * Ss[1])
        ...
        # Creating A matrix
        # Upper diag
        Aupper = np.diag(np.ones(self.Nz-1) * self.Kv / self.dz, 1)
        # Lower diag
        Alower = np.diag(np.ones(self.Nz-1) * self.Kv / self.dz, -1)
    ...
    # Building b matrix
    def build_rhs_vector(self, h_matr, Ss, precon):
    ...
    def iterate(self, h_matr, precons_head):
        ...
        while np.sum(Cell_change > self.CC) > 0:
            ...
            fdm = SolveFDM(self.Nz, self.n, self.dz, self.Kv, self.Sskv,
                           self.Sske, self.dt, precons_head, self.CC,
                           self.toplay)

            # Builds A matrix and updates Ss and preconsolidated head
            A, Ss, precons_head = fdm.build_coeff_matrix(h_matr)

            # Builds right hand side array
            b = fdm.build_rhs_vector(h_matr, Ss, precons_head)

            # Solves for head using A and RHS matrix b
            h_new = fdm.solve_linear_system(A, b)
...
def bkk_postproc(wellnestlist, sub_total, subv_total):
...
def bkk_plot(path, wellnestlist, all_results,
    ...
    for num_well, wellnest in enumerate(wellnestlist):
        ...
        if benchflag == 1:
            ...
            loc = abs_path + "\\inputs\\SurveyingLevels.xlsx"
            try:
        ...
        if benchflag == 1:
            ...
            if not bench.empty:
                ...
                plt.bar(x+width, plot_data[
                    plot_data.columns[
                        ...
                        label="Leveling", width=width, edgecolor='k')
            ...
            plt.xlabel("Years")
            plt.title(wellnest
                      + "\nObserved vs. Simulated Annual Subsidence Rate"
                      + "\nRMSE: " + '{:.1f}'.format(rms) + " cm")
            plt.annotate("RMSE: " + '{:.1f}'.format(rms) + " cm",
...
all_results, sub_total, subv_total = bkk_subsidence(wellnestlist,
                                                    mode, tmin,
                                                    tmax,
                                                    Thick_data,
                                                    Sskv_data,
...
# Post process data
sub_total, subv_total, ann_sub = bkk_postproc(wellnestlist, sub_total,
                                              subv_total)
...
# Plots results
bkk_plot(path, wellnestlist, all_results,
         sub_total, subv_total, ann_sub,
         tmin=tmin, tmax=tmax, save=0,
         benchflag=1)
