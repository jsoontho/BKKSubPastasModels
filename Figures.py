...
full_path = os.path.join(abs_path, 'inputs\BasinPumping.xlsx')
...
plt.plot(pump_50.Date, pump_50_25.Pump2, linewidth=1.5, label='Delayed\n250,000 m$^3$/day')
...
plt.xlabel("Year"); plt.ylabel("Pumping Rate (m$^3$/day)")
...
# Plotting
plt.figure(figsize=(3.2, 2.2), dpi=300)
plt.plot(x, y[:100], linewidth=1.5)
plt.grid(True, linestyle='dotted')
plt.xlabel("Days"); plt.ylabel("Groundwater Head (m)")
...
EstTotPump = pd.read_excel(pumppath, sheet_name=pumpsheet, index_col=0, parse_dates=['Date'])
EstTotPump_ = ps.StressModel(EstTotPump.Pump, rfunc=pump_rfunc, name="well", settings="well", up=False)
...
plt.grid(True, linestyle='dotted')
plt.legend()
plt.xlabel("Years"); plt.ylabel("Groundwater Head (m)")
# plt.title("LCBKK003 NL34\nGroundwater Forecasts\nBased on Pumping Scenarios")

## saving
path = "figures"
fig_name = 'GWForecasts_LCBKK003NL34.png'
full_figpath = os.path.join(path, fig_name)
plt.savefig(full_figpath, bbox_inches='tight', format="png")
...
axs[0].plot(pump_2020.Date[:24472], pump_2020.Pump2[:24472], linewidth=1.5)
...
all_head_data, gw_well_head = js_mfs.GW_Data_Process(data, well_name)
...
axs[1].set_title("Groundwater Levels for Well Nest LCBKK003 and Well BK2")
axs[1].grid(True, linestyle='dotted')
loc = "inputs\SurveyingLevels.xlsx"
...
axs[2].set_title("Annual Land Subsidence Rates from Benchmark Leveling Station DMR62")
plt.tight_layout()
